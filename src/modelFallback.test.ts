...
describe("modelDownloadUrls", () => {
  it("returns the single onnx URL for a sapiens2 variant", () => {
    ...
    expect(urls[0]).toBe(
      `${HF_HOST}/barakplasma/sapiens2-onnx/resolve/main/sapiens2_0.1b_int8.onnx`,
    );
  });
  ...
  it("lists the vision repo files for nomic", () => {
    ...
    expect(
      urls.some((u) =>
        u.endsWith("/nomic-embed-vision-v1.5/resolve/main/config.json"),
      ),
    ).toBe(true);
...
describe("buildUploadCache", () => {
  ...
  it("returns undefined for files that were not uploaded", async () => {
    const cache = buildUploadCache(
      new Map([["repo/config.json", fileFor("config.json")]]),
    );
...
describe("isDownloadError", () => {
  ...
  it("matches our sapiens2 download error message", () => {
    expect(isDownloadError(new Error("Sapiens2 download failed: 404"))).toBe(
      true,
    );
  });
  it('matches transformers "could not locate" errors', () => {
    expect(
      isDownloadError(new Error("Could not locate file: config.json")),
    ).toBe(true);
  });
  ...
  it("does not flag a bare TypeError from a programming bug", () => {
    ...
    expect(isDownloadError(new TypeError("foo.bar is not a function"))).toBe(
      false,
    );
    expect(
      isDownloadError(
        new TypeError("Cannot read properties of undefined (reading 'run')"),
      ),
    ).toBe(false);
  });
});
