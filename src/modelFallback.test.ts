...
describe('modelDownloadUrls', () => {
  ...
  it('lists the vision repo files for nomic', () => {
    ...
    expect(urls.some((u) => u.endsWith('/nomic-embed-vision-v1.5/resolve/main/config.json'))).toBe(
      true
    )
...
describe('buildUploadCache', () => {
  ...
  it('returns undefined for files that were not uploaded', async () => {
    ...
    const miss = await cache.match('https://huggingface.co/x/y/resolve/main/onnx/model.onnx')
...
describe('isDownloadError', () => {
  ...
  it('does not flag a bare TypeError from a programming bug', () => {
    ...
    expect(
      isDownloadError(new TypeError("Cannot read properties of undefined (reading 'run')"))
    ).toBe(false)
  })
})
